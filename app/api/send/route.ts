...
// `inicio`/`fim` sao apenas a barra decorativa do topo; `acento` e o unico
// usado como texto ou borda e precisa contrastar com o fundo branco do e-mail.
const ORIGEM_CORES: Record<Origem, { inicio: string; fim: string; acento: string }> = {
  home: { inicio: '#1A1A1A', fim: '#8A8A8A', acento: '#1A1A1A' },
  tech: {
    inicio: DIVISAO_CONFIG.tech.hexInicio,
    fim: DIVISAO_CONFIG.tech.hexFim,
    acento: DIVISAO_CONFIG.tech.hexAcento,
  },
  sports: {
    inicio: DIVISAO_CONFIG.sports.hexInicio,
    fim: DIVISAO_CONFIG.sports.hexFim,
    acento: DIVISAO_CONFIG.sports.hexAcento,
  },
  studio: {
    inicio: DIVISAO_CONFIG.studio.hexInicio,
    fim: DIVISAO_CONFIG.studio.hexFim,
    acento: DIVISAO_CONFIG.studio.hexAcento,
  },
};
...
function emailInterno(dados: {
  ...
  const { inicio, fim, acento } = ORIGEM_CORES[dados.origem];
  ...
  <div style="background:#F5F5F5;padding:32px 16px;">
    <div style="max-width:560px;margin:0 auto;background:#FFFFFF;border-radius:8px;overflow:hidden;">
      ...
      <div style="padding:32px;">
        <p style="margin:0 0 4px;font:700 11px/1.4 Helvetica,Arial,sans-serif;letter-spacing:0.16em;text-transform:uppercase;color:${acento};">${escapeHtml(ORIGEM_LABEL[dados.origem])}</p>
  ...
  <div style="background:#F5F5F5;padding:32px 16px;">
    <div style="max-width:560px;margin:0 auto;background:#FFFFFF;border-radius:8px;overflow:hidden;">
      ...
      <div style="padding:40px 32px;">
        ...
        <div style="margin:0 0 24px;padding:16px 20px;background:#FAFAFA;border-left:3px solid ${acento};">
        ...
        <p style="margin:0 0 28px;font:400 15px/1.7 Helvetica,Arial,sans-serif;color:#4A4A4A;">
          ...
          <a href="${socialLinks.whatsapp}" style="color:${acento};font-weight:600;">WhatsApp</a>.
        </p>
